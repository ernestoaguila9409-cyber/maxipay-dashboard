...
class SelectTaxesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection("Taxes").get()
            .addOnSuccessListener { snap ->
                ...
                for ((taxId, taxName, taxDetail) in taxes) {
                    ...
                    val cb = CheckBox(this).apply {
                        text = "$taxName ($taxDetail)"
                        textSize = 15f
                        setTextColor(Color.parseColor("#1E293B"))
                        isChecked = preSelected.contains(taxId)
                    }
                    card.addView(cb)
                    checkBoxes[taxId] = cb
