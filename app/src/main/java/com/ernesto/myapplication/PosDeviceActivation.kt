...
/**
 * Redeems a dashboard-generated code in `Merchants/{mid}/deviceActivations/{code}`.
 *
 * Lists all merchants, then checks each one for a doc whose id equals the entered code.
 * The merchant id is persisted locally and used to initialise [MerchantFirestore].
 */
object PosDeviceActivation {
        ...
        fun afterAuthReady() {
            db.collection("Merchants").get()
                ...
                .addOnSuccessListener { merchantSnap ->
                    ...
                    findCodeAcrossMerchants(db, merchantSnap.documents.map { it.id }, code, 0, context, appCtx, onSuccess, onError)
                }
        }
    ...
    private fun findCodeAcrossMerchants(
        ...
        db.collection("Merchants").document(merchantId)
            .collection("deviceActivations").document(code)
            .get()
                ...
                PosDeviceIdentity.resolveInstallationDocId(context) { installationDocId ->
                    ...
                    db.runTransaction { tx ->
                        ...
                        val chain = buildString {
                            var t: Throwable? = e; var n = 0
                            while (t != null && n++ < 8) { t.message?.let { append(it).append(' ') }; t = t.cause }
                        }
                        val msg = when {
