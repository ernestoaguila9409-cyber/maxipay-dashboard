...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      console.log("[Auth] State changed →", firebaseUser ? `UID: ${firebaseUser.uid}, email: ${firebaseUser.email}` : "signed out");
      setUser(firebaseUser);
      if (firebaseUser) {
        const tokenResult = await firebaseUser.getIdTokenResult();
        const c: CustomClaims = {
          role: tokenResult.claims.role as CustomClaims["role"],
          merchantId: tokenResult.claims.merchantId as string | undefined,
        };
        setClaims(c);
        console.log("[Auth] Custom claims →", c);
      } else {
        setClaims({});
      }
      setLoading(false);
