...
export default function TableLayoutEditorClient() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      merchantCol(merchantId, SECTIONS_COLLECTION),
      (snap) => {
  ...
  const persistTable = useCallback(
    async (id: string, data: TableLayoutTableDocument) => {
      ...
      setSaveState("saving");
      try {
        await upsertLayoutTable(db, merchantId, layoutId, id, data);
        setSaveState("saved");
  ...
  const renameLayout = async () => {
    ...
    setSaveState("saving");
    try {
      await updateTableLayoutMeta(db, merchantId, layoutId, { name: name.trim() });
      setSaveState("saved");
      setTimeout(() => setSaveState("idle"), 1200);
    } catch {
      setSaveState("error");
  ...
  const removeSectionFromWeb = async (row: SectionRow) => {
    ...
    setSaveState("saving");
    try {
      await deleteFirestoreSectionDoc(db, merchantId, row.id);
  ...
  const addTable = async () => {
    ...
    setSaveState("saving");
    try {
      await upsertLayoutTable(db, merchantId, layoutId, null, next);
      setSaveState("saved");
      setTimeout(() => setSaveState("idle"), 1200);
    } catch {
      setSaveState("error");
  ...
  const duplicateTable = async () => {
    ...
    setSaveState("saving");
    try {
      await upsertLayoutTable(db, merchantId, layoutId, null, d);
      setSaveState("saved");
      setTimeout(() => setSaveState("idle"), 1200);
    } catch {
      setSaveState("error");
  ...
  const removeTable = async () => {
    ...
    setSaveState("saving");
    try {
      await deleteLayoutTable(db, merchantId, layoutId, selectedId);
      setSelectedId(null);
      setSaveState("idle");
    } catch {
      setSaveState("error");
  ...
  const saveInspector = async () => {
    ...
    setSaveState("saving");
    try {
      await upsertLayoutTable(db, merchantId, layoutId, selected.id, selected.data);
      setSaveState("saved");
      setTimeout(() => setSaveState("idle"), 1200);
    } catch {
      setSaveState("error");
  ...
  const saveCanvasSize = useCallback(async () => {
    ...
    setSaveState("saving");
    try {
      await updateTableLayoutMeta(db, merchantId, layoutId, {
